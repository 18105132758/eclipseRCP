 ...
 *
 */
public class PieDiagram_local extends ViewPart implements ISelectionListener, IPropertyChangeListener{
...
//		PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().addSelectionListener(this);
	...
	private  PaintListener clearLintener;
	private  PaintListener uiLintener;
	private  PaintListener piLintener;
	private void clearListener() {
		if(clearLintener != null) {
			canvas.removePaintListener(clearLintener);
		}
		if(uiLintener != null) {
			canvas.removePaintListener(uiLintener);
		}
		if(uiLintener != null) {
			canvas.removePaintListener(piLintener);
		}
	}
	private void drawDiagram(Composite parent, int year, String unitId) {
		...
		clearLintener = new PaintListener() {
			@Override
		...
		canvas.addPaintListener(clearLintener);
		
//		canvas.redraw();
		
		YearUnitJF jf = YearTeachUnitService.INSTANCE.getYearUnitJF(unitId, year);
		canvas.addPaintListener(new DrawArc(5, 5, 0, 360, 0, bgColor, 0, 0, 0, 0, 0));
		double ui = jf.getUi();
		double pi = jf.getPi();
		double ci = jf.getCi();
		...
		//绘制各类经费
		uiLintener = new DrawArc(0, 0, 0, uiPer, 1, uibgColor, ui, pi, ci, ryjf, zhywf);
		canvas.addPaintListener(uiLintener);
		piLintener = new DrawArc(0, 0, uiPer, piPer, 2, pibgColor, ui, pi, ci, ryjf, zhywf);
		canvas.addPaintListener(piLintener);
